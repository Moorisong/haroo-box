...
### 백엔드 폴더 구조 (apps/server/src)
```
apps/server/src
│
├─ config
│   ├─ db.ts               # MongoDB 연결 설정
│   └─ passport.ts         # 카카오 세션 및 인증 설정
│
├─ models
│   ├─ user.model.ts            # 사용자 스키마
│   ├─ puzzle.model.ts          # 퍼즐 목록 스키마
│   ├─ puzzle-result.model.ts   # 최종 완료 랭킹 기록 스키마
│   ├─ puzzle-progress.model.ts # 실시간 진행 상황 스키마
│   ├─ challenge-token.model.ts # 챌린지 시작 시 1회용 토큰 스키마
│
├─ controllers
│   ├─ puzzle.controller.ts
│   ├─ ranking.controller.ts
│   ├─ puzzle-progress.controller.ts
│   └─ user.controller.ts
│
├─ routes
│   ├─ puzzle.routes.ts
...
│
├─ middlewares
│   ├─ puzzle-auth.ts          # 일반 사용자 JWT/세션 검증 미들웨어
│   └─ verify-challenge.ts     # 챌린지 토큰 및 치팅 방지 미들웨어
│
├─ app.ts                      # Express 서버 인스턴스 초기화 및 로드
└─ index.ts                    # 서버 포트 리스닝 시작점
```
...
#### 1) 현재 활성화된 주간 퍼즐 조회
* **HTTP Method & Path**: `GET /api/puzzle/current`
* **인증 여부**: 비인증 가능
* **성공 응답 (200 OK)**:
```json
{
...
#### 2) 아카이브 퍼즐 목록 조회
* **HTTP Method & Path**: `GET /api/puzzle/archive`
* **인증 여부**: 비인증 가능
* **성공 응답 (200 OK)**:
```json
{
...
#### 3) 특정 퍼즐 상세 조회
* **HTTP Method & Path**: `GET /api/puzzle/:id`
* **인증 여부**: 비인증 가능
* **성공 응답 (200 OK)**:
```json
{
